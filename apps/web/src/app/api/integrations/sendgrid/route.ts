...
import { sendGridConfigSchema } from "@v1/supabase/validations/integrations";
...
export async function GET(request: NextRequest) {
  try {
    console.log('=== SendGrid GET Debug ===');
    ...
    const config = await integrationManager.getSendGridConfig();
    ...
    if (!config) {
      ...
      return NextResponse.json(
        { 
          isConfigured: false,
          message: "No SendGrid configuration found"
        },
        { status: 200 }
      );
    }
    ...
    const safeConfig = {
      ...
      customSubject: config.custom_subject || "",
      isConfigured: true,
    ...
    console.error("Error retrieving SendGrid configuration:", error);
...
export async function POST(request: NextRequest) {
  try {
    console.log('=== SendGrid POST Debug ===');
    console.log('Request headers:', Object.fromEntries(request.headers.entries()));
    ...
    console.log('Request body received:', { ...body, apiKey: body.apiKey ? '***' : 'undefined' });
    
    // Validate request body
    const validation = sendGridConfigSchema.safeParse(body);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const integrationManager = createIntegrationManager(userId, supabase);
    const result = await integrationManager.deleteConfiguration('sendgrid');
    ...
    return NextResponse.json(
      { 
        success: true, 
        message: "SendGrid configuration deleted successfully"
      },
      { status: 200 }
    );
