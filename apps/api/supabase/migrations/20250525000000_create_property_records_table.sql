...
CREATE TABLE public.property_records (
    ...
    portfolio_purchased_last_6_months INTEGER,
    portfolio_purchased_last_12_months INTEGER,
    years_owned INTEGER,
...
CREATE POLICY select_own_property_records ON public.property_records
FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY insert_own_property_records ON public.property_records
FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY update_own_property_records ON public.property_records
FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY delete_own_property_records ON public.property_records
